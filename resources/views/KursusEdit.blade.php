    ...
    <section>
        ...
        <form action="{{ url('/kursus/'.$kursus->id_kursus) }}" method="POST">
            ...
            <div class="flex flex-col mb-3">
                ...
                <input value="{{ $kursus->judul_kursus }}" name="judul_kursus" type="text" placeholder="Judul"
                    class="input input-sm input-bordered w-full max-w-sm" />
            </div>
            <div class="flex flex-col mb-3">
                <label for="deskripsi_kursus">Deskripsi Kursus</label>
                <textarea name="deskripsi_kursus" class="textarea textarea-bordered max-w-sm" placeholder="Deskripsi">{{ $kursus->deskripsi_kursus }}</textarea>
            </div>
            <div class="flex flex-col mb-3">
                <label for="durasi_kursus">Durasi ( <i>Hari</i> )</label>
                <input value="{{$kursus->durasi_kursus}}" name="durasi_kursus" type="number" min="1" placeholder="Durasi"
                    class="input input-sm input-bordered w-full max-w-sm" />
            </div>
